...
"""
This module provides user configuration file management features for Ezcad
...
# Local imports
from ezcad.config.base import (get_conf_path, get_home_dir,
                               get_module_source_path, TEST)
from ezcad.utils.programs import check_version
...
class DefaultsConfig(cp.ConfigParser):
    ...
    def _set(self, section, option, value, verbose):
        ...
        if not isinstance(value, str):
            value = repr( value )
        if verbose:
            print('%s[ %s ] = %s' % (section, option, value))  # ezcad: test-skip
    ...
    def _filename_global(self):
        ...
        if self.subfolder is None:
            ...
            # Save defaults in a "defaults" dir of .ezcad2 to not pollute it
...
class UserConfig(DefaultsConfig):
    ...
    def _load_old_defaults(self, old_version):
        ...
        if check_version(old_version, '3.0.0', '<='):
            path = get_module_source_path('ezcad')
        else:
