...
# Local imports
from ezcad.config.base import get_image_path, running_in_mac_app
from ezcad.config.gui import get_shortcut
from ezcad.utils import programs
from ezcad.utils import icon_manager as ima
from ezcad.utils.icon_manager import get_icon, get_std_icon
...
def qapplication(appname='EZCAD', translate=True, test_time=3):
    """
    ...
    if running_in_mac_app():
        EzcadApplication = MacApplication
    else:
        EzcadApplication = QApplication

    app = EzcadApplication.instance()
    if app is None:
        # Set Application name for Gnome 3
        # https://groups.google.com/forum/#!topic/pyside/24qxvwfrRDs
        app = EzcadApplication([appname])
...
def create_toolbutton_help(parent, triggered=None):
    button = create_toolbutton(parent, icon=ima.icon('help'),
        tip=_('Info on how to use'), triggered=triggered)
    return button
...
def create_action(parent, text, shortcut=None, icon=None, tip=None,
    ...
    """Create a QAction"""
    action = EzcadAction(text, parent)
    if triggered is not None:
...
def add_actions(target, actions, insert_before=None):
    ...
    for action in actions:
        if (action is None) and (previous_action is not None):
            ...
            if isinstance(action, EzcadAction):
...
def create_module_bookmark_actions(parent, bookmarks):
    ...
    for key, url, title in bookmarks:
        # Create actions for scientific distros only if Ezcad is installed
        # under them
        create_act = True
        if key == 'winpython':
...
class EzcadAction(QAction):
    """Ezcad QAction class wrapper to handle cross platform patches."""

    def __init__(self, *args, **kwargs):
        """Ezcad QAction class wrapper to handle cross platform patches."""
        super(EzcadAction, self).__init__(*args, **kwargs)
    ...
    def __getattribute__(self, name):
        ...
        attr = super(EzcadAction, self).__getattribute__(name)
