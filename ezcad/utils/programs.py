...
# Local imports
from ezcad.utils.functions import to_unicode_from_fs
...
if os.name == 'nt':
    TEMPDIR = tempfile.gettempdir() + osp.sep + 'ezcad'
else:
    username = to_unicode_from_fs(getuser())
    TEMPDIR = tempfile.gettempdir() + osp.sep + 'ezcad-' + username
...
def check_version(actver, version, cmp_op):
    ...
    if is_stable_version(version) and not is_stable_version(actver) and \
      ...
      version.startswith(actver) and version != actver:
        ...
        print("version require {}, active {}".format(version, actver))
